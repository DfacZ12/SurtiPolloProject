...
const ListClients = () => {
  ...
  const handleListClient = async () => {
    try {
      const response = await axios.get(`${API_URL}/listClients`, {
  ...
  return (
    <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
      <table className="w-full text-sm text-left rtl:text-right text-gray-500 ">
        <thead className="bg-gray-800 whitespace-nowrap text-center">
          <tr>
            ...
            <th className="p-4 text-sm font-medium text-white">Telefono Fijo</th>
            <th className="p-4 text-sm font-medium text-white">Direccion</th>
        ...
        <tbody className="whitespace-nowrap">
          {ClientList.length === 0 ? (
            ...
            ClientList.map((Client: IClient, index) => (
              <tr
                key={index}
              ...
              >
                ...
                <td className="p-4 text-center text-sm text-black">
                  {formatFecha(Client.fecha_creacion)}
                </td>
                <td className="p-4 text-center">
                  ...
                  <Tooltip content="Eliminar Usuario" side="top">
                    <button
                      ...
                      onClick={() => confirmDeleteClient(Client.cedula)}
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
      ...
      >
        <UpdateClient
          cedula={selectedClient}
          onClose={() => setSelectedClient(null)}
          onUpdated={() => handleListClient()}
        />
      </Modal>
    </div>
  );
