...
const UpdateClient = ({ cedula, onClose, onUpdated }: { cedula: number | null, onClose: () => void, onUpdated: () => void }) => {
  const auth = useAuth();
  const [productInfo, setproductInfo] = useState<IClient[] | []>([]);
  ...
  const handleSelectClient = async () => {
    try {
      const response = await axios.get(`${API_URL}/selectClient/${cedula}`, {
